...
3. Save it as `DATABASE_URL` in your deployment environments.
...
2. Connect this GitHub repo (`endocrineai-hub/endocrine-ai`).
...
5. Deploy. The start command is:
   - `gunicorn run:app --bind 0.0.0.0:$PORT --workers 2 --threads 4 --timeout 120`

### 3. Vercel Deployment

This repo includes [`vercel.json`](vercel.json) and [`api/index.py`](api/index.py) for Python deployment.
